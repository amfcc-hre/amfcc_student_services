(function () {
  ...
  function modeMessage(mode) {
    if(mode==="conference") return "Conference Mode: meal planning has no deadline, manual-work sessions are disabled, and every active task is an Emergency.";
    ...
    return "Normal Mode: standard meal deadlines, gate-pass rules, and manual-work sessions apply.";
  }
  function renderMode() {
    if(!modeData) return; el("operatingMode").value=modeData.mode; el("holidayMode").checked=modeData.mode==="holiday"; el("holidayRuleSummary").textContent=modeMessage(modeData.mode);
  }
  ...
  async function refreshMode(){if(!controlSession)return;var data=await call("system_control_bootstrap",{p_session_token:controlSession.session_token});if(!data||data.status!=="success"){controlSession=null;sessionStorage.removeItem("amfcc_admin_control_session");return}modeData={mode:data.mode};renderMode()}
  async function saveSettings(event) {
    ...
    try{
      ...
      var name=actor(),mode=el("operatingMode").value;
      ...
      modeData={mode:mode};renderMode();showMessage("Operating mode and Admin settings saved.",false);
  ...
  document.addEventListener("DOMContentLoaded",function(){
    ...
    el("operatingMode").addEventListener("change",function(){el("holidayMode").checked=this.value==="holiday";el("holidayRuleSummary").textContent=modeMessage(this.value)});
